...
from capx.deadline import rebase_active_deadline
...
def _run_single_trial(
    ...
    # --- 1. Reset environment ---
    # The historical CaP-X CLI and LIBERO adapter both use a 1-based simulator
    # seed. Controlled manifests use a separate 0-based public key. The runner
    # keeps the internal seed at ``public + 1`` and registers the public key
    # independently.
    seed = trial + int(os.getenv("CAPX_SEED_OFFSET", "0"))
    registered_key = os.getenv("CAPX_REGISTERED_EPISODE_KEY", "").strip()
    episode_base = os.getenv("CAPX_EPISODE_KEY", "").split("/seed", 1)[0]
    if registered_key:
        os.environ["CAPX_EPISODE_KEY"] = registered_key
    elif episode_base:
        os.environ["CAPX_EPISODE_KEY"] = f"{episode_base}/seed{seed}"
    obs, _ = env.reset(options={"trial": trial, "manifest_seed": seed}, seed=seed)
    ...
    # Preserve the wrapper's periodic re-arm.  A one-shot SIGALRM can be
    # swallowed by third-party callbacks (for example JAX XLA GC), which
    # would otherwise remove the episode deadline entirely.
    rebase_active_deadline(1000.0)  # restart fresh 1000s from now
