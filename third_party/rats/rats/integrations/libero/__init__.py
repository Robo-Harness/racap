...
def trial_seed_from_public_seed(public_seed: int) -> int:
    """Translate a zero-based benchmark seed into RATS' trial seed.

    RATS historically numbers lifelong iterations from one, while the
    controlled benchmark exposes seeds from zero.  Keeping this conversion in
    one small, tested function prevents the first fixed-task iteration from
    silently falling back to LIBERO init state zero for every public seed.
    """

    return int(public_seed) + 1
...
class LiberoHandle:
    ...
    def reset(self, seed: int | None = None) -> tuple[Any, dict[str, Any]]:
        ...
        if self.init_states is not None:
            # RATS trial seeds are one-based: trial seed 1 is LIBERO state 0.
            init_state_index = (
                0 if seed is None else (int(seed) - 1) % len(self.init_states)
            )
