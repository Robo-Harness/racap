...
class FrankaLiberoEnv(BaseEnv):
    ...
    def __init__(
        ...
        controlled_instruction = os.environ.get("CONTROLLED_TASK_INSTRUCTION", "").strip()
        if controlled_instruction:
            # The controlled-comparison manifest resolves LIBERO-PRO task
            # perturbations whose BDDL language changed while filenames did not.
            self.handle.task_language = controlled_instruction
    ...
    def reset(
        ...
        # The handle applies the registered init state and returns its observation.
        libero_obs, libero_info = self.handle.reset(seed=seed)
        self._current_init_state_index = libero_info.get("init_state_index")

        # LiberoHandle is the single authority that maps the one-based RATS
        # trial seed to a serialized LIBERO init state.  Do not call raw
        # ``env.reset`` or ``set_init_state`` again here: either would apply a
        # second scene transition and could make telemetry disagree with the
        # state actually executed.
