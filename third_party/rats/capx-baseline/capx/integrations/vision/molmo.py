...
def init_molmo(
    ...
    runtime_url = os.getenv("CAPX_RUNTIME_VLM_URL", "").strip()
    runtime_model = os.getenv("CAPX_RUNTIME_VLM_MODEL", "").strip()
    if runtime_model:
        model_name = runtime_model
    chat_url = runtime_url or f"{base_url.rstrip('/')}/chat/completions" # vLLM
    session = requests.Session()
    headers = {"Content-Type": "application/json"}
    api_key = os.getenv("CAPX_RUNTIME_VLM_KEY") or api_key
    if api_key:
    ...
    def det_fn(
        ...
        for obj in objects:
            prompt = (
                f"Point at {obj}. Reply only as <point x=\"X\" y=\"Y\"> "
                "where X and Y are percentages from 0 to 100 of image width and height."
            )
            payload = {
                "model": model_name,
                "messages": [
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if runtime_url:
                        from capx.llm.client import _append_call_telemetry

                        _append_call_telemetry(
                            {
